<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<div class="modal fade" id="standar_popvar" tabindex="-1" role="dialog" aria-labelledby="examquestion_plusLabel" aria-hidden="true">
				...
				<div class="po_r" id="style-3" >
					<table class="table table-sss02 wsize_100">
						...
						<tr>
							...
							<td class="text-left">
								<span v-for="ss in selecColumn" placeholder="aaa">{{ss.columnName}}</span>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<th> <span class="red02">＊</span>표준화 컬럼설명(자동생성)</th>
							<td class="text-left">
								<span>{{standarLableName}}</span>
							</td>
						</tr>
					</table>
				</div>
					...
					<div class="wsize_50 fl div_bg">변경 할 값</div> <div class="wsize_50 fl div_bg">변경 후 값</div>
						<div class ="mt_10 div_set" style ="height: 25vh; width: 100%;">				
				            <div class ="wsize_100 fl pd_10 hwsize_30 mt_10">
				            	<select class="form-control class_select100 fl size_280" id="processList">
								...
								<i class="fa fa-chevron-right fl pd_10"></i>
								<i class="fa fa-chevron-right fl pd_10"></i>								
								<select class="form-control class_select100 fl size_280" id="processList2">
									<option v-for="(proc, index) in processList" v-bind:value="index" >{{index}}</option>									
								</select>
								<input type="button" class="fl btn-creat size_80 btn_set" value="추가" v-on:click="processAdd(1)"> 
				            </div>
				            ...
				            <div class = "wsize_100 fl pd_10 hwsize_30">
								...
								<i class="fa fa-chevron-right fl pd_10"></i>
								<i class="fa fa-chevron-right fl pd_10"></i>						
				            	<input type="number" class="form-control size_280 fl" min="0" max="100" id="null" value="0">
				            	<input type="button" class="fl btn-creat size_80 btn_set" value="추가" v-on:click="processAdd(3)">
				            </div>
				            ...
				            <div class = "wsize_100 fl pd_10 hwsize_30">
								...
								<i class="fa fa-chevron-right fl pd_10"></i>
								<i class="fa fa-chevron-right fl pd_10"></i>
								<input type="number" class="form-control size_280 fl" min="0" max="100" id="else" value="0">
...
<!-- 모달footer -->
			<div class="modal-footer">
				<button type="button" class="btn btn-cencle" v-on:click="cancelPopup(3)">닫기</button>
				<button type="button" class="btn btn-creat" v-on:click="saveStandar(3)">저장</button>
			</div>
		</div>
	</div>
</div>
